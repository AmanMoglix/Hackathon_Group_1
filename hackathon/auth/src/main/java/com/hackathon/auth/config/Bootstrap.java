...
public class Bootstrap implements CommandLineRunner {
    ...
    private void saveRoles() {
        ...
        if (!roleService.isRoleExist(ApplicationRole.SUPPLIER.toString()))
            roleService.save(ApplicationRole.SUPPLIER.toString());
        if (!roleService.isRoleExist(ApplicationRole.MANAGER.toString()))
            roleService.save(ApplicationRole.MANAGER.toString());
        if (!roleService.isRoleExist(ApplicationRole.CUSTOMER.toString()))
            roleService.save(ApplicationRole.CUSTOMER.toString());
    }
...
//            userService.mapLocation(userLocationCO,"superadmin@gmail.com");
        ...
        if (!userService.isExistByUsername("supplier@gmail.com")) {
            UserCO userCO = new UserCO("supplier@gmail.com", "Admin@123",
                    "Sales Person", "Authority", "ROLE_SUPPLIER");
            String userId = userService.save(userCO);
