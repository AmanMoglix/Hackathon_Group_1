...
public class DiscoveryServiceImpl implements DiscoveryService {
	...
	public SupplierDiscoveryDTO fetchBySupplierId(Long supplierId, CurrentUser currentUser) {
		logger.info("Going to fetch Supplier by supplierId '{}'", supplierId);
		try {
			String PATH = "/api/" + supplierId;
			String url = getServiceInstance(this.applicationConfig.getAuthServiceId()).getUri() + PATH;
			HttpHeaders headers = getHeaders(false, currentUser);

			logger.info("Requesting to fetch supplier by supplierId");
			logger.info("Request URL for Supplier by supplierId :{}", url);
			logger.info("Request Headers for Supplier by supplierId:{}", new Gson().toJson(headers));
			ResponseEntity<SupplierDiscoveryDTO> responseEntity = this.restTemplate.exchange(url, HttpMethod.GET,
					new HttpEntity<>(headers), new ParameterizedTypeReference<SupplierDiscoveryDTO>() {
					});
			int httpStatus = responseEntity.getStatusCodeValue();
			if (httpStatus == HttpStatus.OK.value()) {
				SupplierDiscoveryDTO response = responseEntity.getBody();
				return response == null ? new SupplierDiscoveryDTO() : response;
			}
		} catch (HttpClientErrorException.BadRequest | HttpClientErrorException.Conflict
				| HttpClientErrorException.NotFound exception) {
			exception.printStackTrace();
			...
			throw new DiscoveryRequestException(
					"Supplier Service error during request of fetching product by productId.");
		}
		return new SupplierDiscoveryDTO();
	}
