...
public class TransacrionHeadServiceImpl implements TransactionHeadService {
				...
				for (TransactionDetails txn : txnDetails) {
					...
					if (txn.getProductId() != null && txn.getProductId() > 0 && txn.getQuantity() > 0) {
						...
						if (stock != null) {
							...
							stk.setUserId(Long.valueOf(currentUser.getUserId()));
	...
	public List<OnlineProductListingDTO> productListing(CurrentUser currentUser) {
		List<OnlineProductListingDTO> onlineProductListingDTOs= this.transactionHeadRepository.productListbyProductPrice();
		onlineProductListingDTOs.forEach(it->{
			if(it!=null)
				it.setCatalog(this.discoveryService.fetchByProductId(it.getProductId(), currentUser));
			it.setSupplier(this.discoveryService.fetchBySupplierId(it.getSupplierId(), currentUser));
			it.setStockId(this.stockService.findByProductIdAndSupplerId(it.getProductId(), it.getSupplierId(), currentUser).getId());
		});
